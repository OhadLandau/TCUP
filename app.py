...
def _z_two_tail_p(z: float) -> float:
    cdf = 0.5 * (1.0 + math.erf(z / SQRT2)); return 2.0 * (1.0 - cdf)
...
def make_bar(prob):
    ...
    fig.update_layout(showlegend=False, xaxis_tickangle=-90,
                      yaxis_title="Probability", title="Predicted class probabilities",
                      margin=dict(b=160))
    return fig
...
def build_summary(label: str, prob: float,
    ...
    if missing:
        ...
        if critical:
            n=len(critical); verb="was" if n==1 else "were"
            extras.append(html.Small(
                ["⚠️ **",str(n),f"** gene(s) {verb} imputed from the top 128 (p < 0.05)."],
                className="warn"))
            extras.append(html.Small(
                [str(n)," significant gene(s): ",", ".join(sorted(critical))],
                className="warn"))
        else:
            extras.append(html.Small("**None** of the imputed genes belong to the top 128 (p < 0.05).",
                                     className="ok"))
    else:
        extras.append(html.Small("No genes required median imputation.", className="ok"))
    return html.Div([
...
def top_gene_list(pred_label,row_log2,med,std) -> Tuple[List[html.Span],html.Small]:
    col=f"AccuracyDrop_{pred_label}"
    if col not in IMP_DF.columns: col="AccuracyDrop"
    top20=IMP_DF[col].sort_values(ascending=False).head(20).index
    spans=[]
    for g in top20:
        expr=row_log2.get(g,med[g]); median=med[g]; s=std.get(g,0) or 1e-9
        z=(expr-median)/s; arrow,cls=("▲","up") if z>0 else("▼","down")
        stars=_p_to_stars(_z_two_tail_p(abs(z)))
        spans.append(html.Span([g,html.Span(f" {arrow}",className=cls),
                                html.Span(f" ({z:+.1f}σ){stars}",className="std")]))
    legend=html.Small("* p < 0.05 ** p < 0.01 *** p < 0.001 (two-tailed z-test).")
    return spans,legend
...
upload_card=html.Div(id="upload-card",style={"display":"none"},children=[
    ...
    dcc.Dropdown(id="sample-select",placeholder="choose sample…",
                 style={"display":"none"}),
    # ---------- hint line ----------
    html.Div(id="hint-msg",children="",style={"minHeight":"22px"}),
...
def handle_upload(contents,fname):
    ...
    hint="The analysis might take a couple of minutes"
    if n==1:
        return df.to_json(date_format="iso",orient="split"),alert,[],None,{"display":"none"},False,hint
    opts=[{"label":str(idx),"value":i} for i,idx in enumerate(df.index)]
    return df.to_json(date_format="iso",orient="split"),alert,opts,None,{"display":"block"},True,hint
...
# -- short callback to update hint immediately when analysis starts
@callback(Output("hint-msg","children"),
          Input("analyze-btn","n_clicks"),prevent_initial_call=True)
def set_running_hint(_):
...
def run_prediction(_,json_df,sample_idx,sample_type):
    ...
    if col not in IMP_DF.columns: col="AccuracyDrop"
    sig=set((IMP_DF.loc[IMP_DF["p_value"]<0.05,col]
             .sort_values(ascending=False).head(128).index))
    summary=build_summary(label,float(probs.max()),missing,sig)
